...
async def extract_session(ctx, page):
    """从浏览器抓 cookies + f.sid/at/dsh/TL/bl。"""
    ...
    url_qs = urllib.parse.parse_qs(urllib.parse.urlparse(page.url).query)
    return {
        "cookies": cookie_dict,
        "fsid": meta.get("fsid"),
        "at": meta.get("at"),
...
async def _login_new_window(bb, proxy, acct, args):
    """在第二个干净窗口登录刚建的账号，拿完整 cookies（SID/HSID 等）。"""
    email = acct["email"]
    password = acct["password"]
    pid2 = None
    try:
        pid2 = bb.create_browser(name="gmail_login", proxy_str=proxy)
        info2 = bb.open_browser(pid2)
        from playwright.async_api import async_playwright
        async with async_playwright() as p:
            br = await p.chromium.connect_over_cdp(info2["ws"])
            ...
            # 检查登录结果
            login_url = page.url
            log(f"[login] 登录后: {login_url[:60]}")

            # 如果停在 challenge/verify = 需要手机验证（新设备），跳过 cookies 导出
            if "challenge" in login_url or "verify" in login_url.lower():
                log("[login] 触发安全验证，无法拿 cookies（动态IP限制）", "WARN")
                acct["cookies"] = await ctx.cookies()
            else:
                # 访问 myaccount 触发 cookies
                await page.goto("https://myaccount.google.com/?hl=en",
                                wait_until="domcontentloaded", timeout=30000)
                await asyncio.sleep(5)
                log(f"[login] myaccount: {page.url[:60]}")
                cookies = await ctx.cookies()
                core = {c['name'] for c in cookies} & {'SID', 'HSID', 'SSID', 'APISID', 'SAPISID'}
                log(f"[login] cookies: {len(cookies)} 条, 核心: {core or '无'}")
                acct["cookies"] = cookies
    except Exception as e:
        log(f"[login] 登录窗口异常: {e}", "WARN")
    finally:
        if pid2:
            try:
                bb.close_browser(pid2)
                await asyncio.sleep(1)
                if args.delete_window:
                    bb.delete_browser(pid2)
            except Exception:
                pass
    return acct


async def _login_and_export_cookies(page, ctx, email, password):
    """在同一浏览器窗口登录刚建的账号，导出完整 cookies（含 SID/HSID/SSID）。"""
    log("[login] 在同窗口登录以获取完整 cookies...")
    # 必须导航到干净的登录页（不能复用注册 flow 的 identifier，会触发 speedbump）
    await page.goto("https://accounts.google.com/signin/v2/identifier?hl=en&flowName=GlifWebSignIn&flowEntry=ServiceLogin",
                    wait_until="domcontentloaded", timeout=30000)
    await asyncio.sleep(3)

    # 填邮箱（click+type 比 fill 更可靠——Google 登录页有 JS 包装）
    email_filled = False
    for sel in ['input#identifierId', 'input[type=email]', 'input[name=identifier]']:
        try:
            loc = page.locator(sel).first
            if await loc.count() and await loc.is_visible():
                await loc.click(timeout=3000)
    ...
    # 填密码（click+type）
    await wait_url(page, "challenge", 12)
    await asyncio.sleep(1.5)
    pw_filled = False
    ...
    # 处理登录后可能出现的中间页（passkey/2FA推荐/欢迎页）→ 跳过
    for _skip in range(5):
        url = page.url
        if "mail.google" in url or "myaccount" in url:
            break
        # 跳过 passkey/recovery/welcome 等中间页
        skipped = False
        for sel in ['button:has-text("Not now")', 'button:has-text("暂不")',
                    'button:has-text("跳过")', 'button:has-text("Skip")',
                    'button:has-text("以后再说")', 'a:has-text("Not now")',
                    'button[jsname="LgbsSe"]']:
            try:
                b = page.locator(sel).first
                if await b.count() and await b.is_visible():
    ...
    # 访问多个 Google 页面触发完整 cookies
    for target in ["https://myaccount.google.com/?hl=en",
                    "https://mail.google.com/mail/u/0/#inbox"]:
        try:
            await page.goto(target, wait_until="domcontentloaded", timeout=30000)
            await asyncio.sleep(5)
            log(f"[login] 访问: {page.url[:60]}")
        except Exception:
            pass
        # 检查是否已拿到核心 cookies
        ck = await ctx.cookies()
        core = {c['name'] for c in ck} & {'SID', 'HSID', 'SSID', 'APISID', 'SAPISID'}
        if core:
            log(f"[login] 拿到核心 cookies: {core}")
            break

    cookies = await ctx.cookies()
    core = {'SID', 'HSID', 'SSID', 'APISID', 'SAPISID'}
    found = {c['name'] for c in cookies} & core
    log(f"[login] cookies: {len(cookies)} 条, 核心: {found or '无'}")
    return cookies
